...
Ans: A retained message is a message which has a flag activated which enables the message to remain for future subscribers. A retained message would be useful for transfering basic data for our smart bin like status(online, offline; back in x hours,...).
...
Ans: The wildcart subscription '#' allows us to observe: the entire data flow of a specified topic, the whole timeline of a bug occuring without having to look at disparate log files, which is critical for time-sensitive production issues. It also helps us in tracking and identifying messages sent to wrong topics, or messages with the wrong data format, so it acts as a 'monitor' of the entire topic. But it can be dangerous in production mainly because of the sheer amount of data being transferred through the broker, which could result in potentially slowing down or crashing the message broker, in the client's inability to process the information, leading to memory saturation and out-of-memory errors, in receiving sensitive client information.  
...
Ans: The subscriber did not receive a message and because the broker had no subcribers the message without the retained flag was not retained by the broker.
